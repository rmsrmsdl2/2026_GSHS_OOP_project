...
class Renderer:
    ...
    def _draw_header(self, world):
        ...
        subtitle = self.tiny_font.render(
            "환경 선택압이 달라질 때 인공 생명체 집단의 형질과 적합도가 어떻게 변하는지 관찰합니다.",
            True,
            self.colors["muted"],
        )
        ...
        img = self.tiny_font.render(controls, True, (184, 194, 187))
        self.screen.blit(img, (560, 54))
    ...
    def _draw_stage_background(self, world):
        ...
        label = self.tiny_font.render("exp(-D) 적합도 기준으로 정렬되는 Biomorph 개체군", True, (170, 181, 174))
    ...
    def _draw_card(self, index, biomorph, selected, rank, score):
        ...
        if rank:
            self._small_badge(rect.x + 8, rect.y + 7, f"상위 {rank}", self.colors["accent"])
        else:
            number = self.tiny_font.render(f"#{biomorph.id}", True, self.colors["muted"])
        ...
        self._bar(bar_x, bar_y, bar_w, 5, score, self.colors["accent"])

    def _draw_analytics_panel(self, world):
        rect = pygame.Rect(18, 568, 920, 168)
        self._panel(rect, "세대별 적응 분석 그래프")
        ...
        left_graph = pygame.Rect(rect.x + 18, rect.y + 44, 420, 92)
        right_graph = pygame.Rect(rect.x + 476, rect.y + 44, 420, 92)
        ...
        summary = f"현재 평균 적합도 {latest['avg_fitness']:.3f}   최고 적합도 {latest['best_fitness']:.3f}   평균 오차 D {latest['avg_error']:.3f}"
        img = self.tiny_font.render(summary, True, (205, 214, 207))
        self.screen.blit(img, (rect.x + 18, rect.bottom - 24))
    ...
    def _draw_environment_panel(self, world, x, y):
        ...
        for i, name in enumerate(FACTOR_NAMES):
            ...
            label = self.tiny_font.render(f"{i + 1}. {FACTOR_LABELS[name]}", True, color)
            self.screen.blit(label, (rect.x + 16, row_y))
        ...
        self._bar(rect.x + 16, rect.bottom - 17, 268, 6, progress, self.colors["accent"])
    ...
    def _draw_environment_graph(self, world, x, y):
        ...
        for name in FACTOR_NAMES:
            values = [row["factors"][name] for row in world.history]
            series.append((FACTOR_LABELS[name], values, colors[name]))
    ...
    def _draw_selected_panel(self, world, x, y):
        ...
        score_img = self.small_font.render(f"적합도 exp(-D): {score:.3f}", True, self.colors["accent"])
        self.screen.blit(score_img, (rect.x + 16, rect.y + 38))
        ...
        for i, (name, gap) in enumerate(important):
            ...
            label = self.tiny_font.render(f"{TRAIT_LABELS[name]}", True, self.colors["text"])
            self.screen.blit(label, (rect.x + 16, row_y))
        ...
        note_img = self.tiny_font.render(note, True, (184, 194, 187))
        self.screen.blit(note_img, (rect.x + 16, rect.bottom - 20))

    def _draw_rank_event_panel(self, world, ranked, x, y):
        rect = pygame.Rect(x, y, 300, 86)
        self._panel(rect, "자연선택 결과 / 로그")

        for i, (biomorph, score) in enumerate(ranked[:5]):
            px = rect.x + 16 + i * 52
            self._small_badge(px, rect.y + 38, f"#{biomorph.id}", self.colors["accent"])
            score_img = self.tiny_font.render(f"{score:.2f}", True, self.colors["muted"])
            self.screen.blit(score_img, (px + 4, rect.y + 60))
    ...
    def _draw_start_page(self, world):
        ...
        subtitle = self.small_font.render(
            "실험을 자동으로 진행할지, 초기 환경 변수를 직접 설정할지 선택하세요.",
            True,
            self.colors["muted"],
        )
        ...
        if world.setup_mode == "manual":
            ...
            self._draw_mode_choice(world)

    def _draw_mode_choice(self, world):
        ...
        guide = self.small_font.render("A: 자동 실행    M: 수동 설정    R: 새 개체군", True, self.colors["muted"])
        self.screen.blit(guide, (64, 650))
    ...
    def _draw_manual_setup(self, world):
        ...
        desc = self.small_font.render(
            "1-4로 환경 요인을 선택하고, 방향키로 값을 조절하세요. Enter를 누르면 이 환경으로 실험을 시작합니다.",
            True,
            self.colors["muted"],
        )
        self.screen.blit(desc, (panel.x + 28, panel.y + 48))
        ...
        for i, name in enumerate(FACTOR_NAMES):
            ...
            label = self.small_font.render(f"{i + 1}. {FACTOR_LABELS[name]}", True, label_color)
            self.screen.blit(label, (panel.x + 36, row_y))
            self._bar(panel.x + 210, row_y + 8, 640, 14, value, colors[name])
            value_img = self.small_font.render(f"{value:.2f}", True, self.colors["text"])
            self.screen.blit(value_img, (panel.x + 880, row_y))
        ...
        msg = self.small_font.render(world.message, True, self.colors["gold"])
        self.screen.blit(msg, (panel.x + 28, panel.bottom - 112))
    ...
    def _choice_card(self, rect, title, body, button, accent):
        ...
        title_img = self.font.render(title, True, self.colors["text"])
        self.screen.blit(title_img, (rect.x + 34, rect.y + 38))

        for i, line in enumerate(body.split("\n")):
            img = self.small_font.render(line, True, self.colors["muted"])
            self.screen.blit(img, (rect.x + 36, rect.y + 98 + i * 28))

        if title == "자동 실행":
            lines = ["- 환경 변화: 자동", "- 세대 진행: 자동", "- 발표 시 전체 흐름 관찰에 적합"]
        else:
            lines = ["- 환경 변화: 직접 설정", "- 세대 진행: 수동", "- 특정 조건 실험에 적합"]

        for i, line in enumerate(lines):
            img = self.small_font.render(line, True, self.colors["text"])
            self.screen.blit(img, (rect.x + 36, rect.y + 190 + i * 30))

        self._button(pygame.Rect(rect.x + 34, rect.bottom - 72, 160, 42), button, accent)

    def _button(self, rect, text, color):
        pygame.draw.rect(self.screen, color, rect, border_radius=8)
        img = self.small_font.render(text, True, (14, 18, 19) if color != self.colors["border"] else self.colors["text"])
    ...
    def _draw_result_page(self, world):
        ...
        title = self.font.render("실험 결과 정리", True, self.colors["text"])
        subtitle = self.small_font.render(
            f"{MAX_GENERATIONS}세대 동안 환경 선택압에 가장 잘 적응한 개체와 우세하게 발달한 형질입니다.",
            True,
            self.colors["muted"],
        )
        ...
        self._metric_pill(995, 30, "우세 개체", f"#{winner.id}", self.colors["gold"])
        self._metric_pill(1140, 30, "적합도", f"{summary['winner_score']:.3f}", self.colors["blue"])

        winner_rect = pygame.Rect(34, 112, 300, 430)
        self._panel(winner_rect, "최종 우세 개체")
        portrait = pygame.Rect(winner_rect.x + 40, winner_rect.y + 78, 220, 230)
        ...
        winner.draw(self.screen, portrait.inflate(-26, -22), selected=True)

        details = [
            f"개체 ID: #{winner.id}",
            f"부모 ID: #{winner.parent_id if winner.parent_id is not None else '초기 개체'}",
            f"최종 적합도: {summary['winner_score']:.3f}",
            f"선택 오차 D: {summary['winner_error']:.3f}",
        ]
        for i, text in enumerate(details):
            img = self.small_font.render(text, True, self.colors["text"] if i == 0 else self.colors["muted"])
            self.screen.blit(img, (winner_rect.x + 24, winner_rect.y + 330 + i * 24))

        spec_rect = pygame.Rect(354, 112, 256, 430)
        self._panel(spec_rect, "우세 개체 스펙")
        most_improved = summary["improved_traits"][0][0]
        spec_title = self.tiny_font.render(f"가장 많이 증가: {TRAIT_LABELS[most_improved]}", True, self.colors["gold"])
        self.screen.blit(spec_title, (spec_rect.x + 18, spec_rect.y + 42))

        for i, name in enumerate(TRAIT_NAMES):
            value = winner.genome.traits[name]
            target = world.environment.optimal_traits[name]
            gap = abs(value - target)
            row_y = spec_rect.y + 72 + i * 36
            label = self.tiny_font.render(TRAIT_LABELS[name], True, self.colors["text"])
            self.screen.blit(label, (spec_rect.x + 18, row_y))
            ...
            text = self.tiny_font.render(f"{value:.2f} / 차이 {gap:.2f}", True, self.colors["muted"])
            self.screen.blit(text, (spec_rect.x + 18, row_y + 17))

        trend_rect = pygame.Rect(632, 112, 280, 230)
        self._panel(trend_rect, "세대별 적합도 변화")
        self._line_graph(
            pygame.Rect(trend_rect.x + 18, trend_rect.y + 54, 244, 118),
            [
                ("평균 적합도", [row["avg_fitness"] for row in world.history], self.colors["accent"]),
                ("최고 적합도", [row["best_fitness"] for row in world.history], self.colors["gold"]),
            ],
        ...
        fit_change = summary["final_avg_fitness"] - summary["initial_avg_fitness"]
        fit_text = f"평균: {summary['initial_avg_fitness']:.3f} -> {summary['final_avg_fitness']:.3f}"
        img = self.small_font.render(fit_text, True, self.colors["accent"] if fit_change >= 0 else self.colors["danger"])
        self.screen.blit(img, (trend_rect.x + 18, trend_rect.bottom - 34))

        error_rect = pygame.Rect(632, 366, 280, 176)
        self._panel(error_rect, "선택 오차 D 변화")
        self._line_graph(
            pygame.Rect(error_rect.x + 18, error_rect.y + 52, 244, 82),
            [("평균 오차 D", [row["avg_error"] for row in world.history], self.colors["danger"])],
            0.0,
            1.0,
        )
        error_change = summary["final_avg_error"] - summary["initial_avg_error"]
        error_text = f"D: {summary['initial_avg_error']:.3f} -> {summary['final_avg_error']:.3f}"
        img = self.small_font.render(error_text, True, self.colors["accent"] if error_change <= 0 else self.colors["danger"])
        self.screen.blit(img, (error_rect.x + 18, error_rect.bottom - 30))

        trait_rect = pygame.Rect(940, 112, 306, 430)
        self._panel(trait_rect, "우세하게 발달한 형질")
        for i, (name, start, end, change) in enumerate(summary["improved_traits"]):
            y = trait_rect.y + 54 + i * 70
            label = self.small_font.render(f"{i + 1}. {TRAIT_LABELS[name]}", True, self.colors["text"])
            self.screen.blit(label, (trait_rect.x + 18, y))
            self._bar(trait_rect.x + 18, y + 28, 210, 9, end, TRAIT_COLORS[name])
            start_x = trait_rect.x + 18 + int(210 * start)
            pygame.draw.line(self.screen, self.colors["muted"], (start_x, y + 22), (start_x, y + 42), 2)
            change_img = self.tiny_font.render(f"{start:.2f} -> {end:.2f} ({change:+.2f})", True, self.colors["accent"])
            self.screen.blit(change_img, (trait_rect.x + 18, y + 43))
        ...
        for i, text in enumerate(conclusion):
            img = self.small_font.render(text, True, self.colors["text"] if i == 0 else self.colors["muted"])
            self.screen.blit(img, (close_rect.x + 22, close_rect.y + 42 + i * 26))
    ...
    def _build_conclusion(self, summary):
        best_trait = summary["improved_traits"][0][0]
        ...
        return [
            f"최종 우세 개체 #{summary['winner'].id}는 현재 환경의 최적 형질에 가장 가까운 개체입니다.",
            f"가장 크게 발달한 형질은 '{TRAIT_LABELS[best_trait]}'이며, 우세 개체는 특히 {close_names} 형질에서 환경에 잘 맞았습니다.",
            f"집단 평균 적합도 변화는 {fitness_delta:+.3f}, 평균 선택 오차 D 변화는 {error_delta:+.3f}입니다.",
        ]
    ...
    def _panel(self, rect, title):
        ...
        self.screen.blit(title_img, (rect.x + 16, rect.y + 13))
    ...
    def _metric_pill(self, x, y, label, value, color):
        ...
        label_img = self.tiny_font.render(label, True, self.colors["muted"])
        value_img = self.small_font.render(value, True, color)
        self.screen.blit(label_img, (x + 10, y + 4))
        self.screen.blit(value_img, (x + 76, y + 6))
